...
class HookRegistryTest {
    ...
    void multipleMatchersOrder() {
        var matcherFormats = new ArrayList<AceCopilotConfig.HookMatcherFormat>();

        // First matcher: all tools
        var hf1 = new AceCopilotConfig.HookConfigFormat("command", "echo first", 0);
        var mf1 = new AceCopilotConfig.HookMatcherFormat(null, List.of(hf1));
        matcherFormats.add(mf1);

        // Second matcher: bash only
        var hf2 = new AceCopilotConfig.HookConfigFormat("command", "echo second", 0);
        var mf2 = new AceCopilotConfig.HookMatcherFormat("bash", List.of(hf2));
        matcherFormats.add(mf2);

        var configMap = new HashMap<String, List<AceCopilotConfig.HookMatcherFormat>>();
    ...
    private static Map<String, List<AceCopilotConfig.HookMatcherFormat>> buildConfigMap(
            String eventName, String matcher, String type, String command, int timeout) {
        var hf = new AceCopilotConfig.HookConfigFormat(type, command, timeout);
        var mf = new AceCopilotConfig.HookMatcherFormat(matcher, List.of(hf));
