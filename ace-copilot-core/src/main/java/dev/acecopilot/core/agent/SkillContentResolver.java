...
public final class SkillContentResolver {
    ...
    private String substituteEnvironmentVars(String body) {
        body = body.replace("${ACE_COPILOT_PROJECT_DIR}", workingDir.toAbsolutePath().toString());

        String sessionId = System.getProperty("ace-copilot.session.id", "");
        body = body.replace("${ACE_COPILOT_SESSION_ID}", sessionId);
