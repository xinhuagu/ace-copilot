...
>
> Copilot's premium-request billing is among the **more opaque** options in the current AI-subscription market for agent workloads — an integer counter with [no per-turn receipt and eventually-consistent usage display](https://docs.github.com/en/copilot/concepts/billing/copilot-requests), a **session endpoint that bills at ~3× the [published per-model multiplier](https://docs.github.com/en/copilot/concepts/billing/copilot-requests)** (observed: Haiku 1× instead of published 0.33×, Sonnet 3× instead of 1×, GPT-5.4 3× instead of 1× — not documented as a rule anywhere), a [silently trimmed context window](docs/copilot-session-runtime.md#3-copilot-caps-claude-context-windows-below-the-models-native-capacity) (~128K effective on Copilot vs 200K native for Sonnet 4.6), and [$0.04/request overage](https://github.blog/changelog/2025-08-22-premium-request-overage-policy-is-generally-available-for-copilot-business-and-enterprise/) that makes short agent turns materially more expensive than the same model direct from Anthropic (a $0.12 Copilot Sonnet session turn ≈ 5× a $0.023 [direct API call](https://www.finout.io/blog/anthropic-api-pricing) for a short prompt; roughly breaks even around 20K input; Copilot is actually *cheaper* on very long context — but Copilot trims the context you'd need to get there). ace-copilot uses Copilot's **own** session SDK — the same underlying mechanism as VSCode Copilot Chat's agent mode — to deliver one-premium-per-user-prompt billing in a CLI/daemon, with telemetry that surfaces the ~3× session multiplier GitHub doesn't show you and [explicit Phase 4 tradeoffs](docs/copilot-phase4-audit.md) for the subsystems that would otherwise silently re-bill you.
